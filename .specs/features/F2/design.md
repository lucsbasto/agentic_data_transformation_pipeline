...
**Revised 2026-04-23:** F1 already ships a `runs` table whose `layer` column accepts `'bronze'|'silver'|'gold'`. Reuse it instead of creating a Silver-specific twin table. The only schema change is adding two generic columns so the table can carry information Silver (and later Gold) need:

```sql
ALTER TABLE runs ADD COLUMN output_path TEXT;
ALTER TABLE runs ADD COLUMN rows_deduped INTEGER;
```

Because ``ensure_schema`` runs ``CREATE TABLE IF NOT EXISTS`` on startup, we migrate existing F1 databases with two defensive ``ALTER TABLE ... ADD COLUMN`` statements guarded by an ``information_schema``-style pragma check (SQLite has no ``IF NOT EXISTS`` on ``ADD COLUMN``, so we query ``PRAGMA table_info(runs)`` and skip the alter when the column is already present).

Effective ``runs`` shape after F2:
...
CREATE TABLE runs (
    ...
    error_type    TEXT,
    error_message TEXT,
    output_path   TEXT,       -- F2: Silver parquet dir; F3: Gold parquet dir
    rows_deduped  INTEGER     -- F2: Silver dedup delta; other layers leave null
);
```

New typed dataclass: `RunRow` (mirrors the above columns) plus helpers that are parameterized on `layer` so Gold reuses them in F3:

- `insert_run(run_id, batch_id, layer, started_at) -> None`
- `mark_run_completed(run_id, rows_in, rows_out, finished_at, duration_ms=None, output_path=None, rows_deduped=None) -> None` *(duration_ms is derived from started_at/finished_at in SQL later; keeping the param optional now so the CLI can pass a monotonic-clock-measured value for precision)*
- `mark_run_failed(run_id, finished_at, error_type, error_message) -> None`
- `get_run(run_id) -> RunRow | None`
- `get_latest_run(batch_id, layer) -> RunRow | None`
- `is_run_completed(batch_id, layer) -> bool` — true when a row exists with `COMPLETED`
- `delete_runs_for(batch_id, layer) -> int` — cleans stale `IN_PROGRESS`/`FAILED` rows before a retry; returns the row count deleted
- `reset_stale_runs(layer=None) -> int` — sweep `IN_PROGRESS` rows at `ensure_schema` time, matching the existing batches sweep

`run_id` is generated by the caller (UUID hex prefix, 12 chars — same convention as the `run_id` correlation id logged in `cli/ingest.py`). The primary key therefore stays the same as F1; multiple Silver rebuilds per Bronze batch become natural in F5 without another migration.
