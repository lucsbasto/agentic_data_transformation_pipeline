...
- ⚪ **F4.1** — `feat(F4): declare agent enums + dataclasses` (`src/pipeline/agent/types.py` com `ErrorKind`, `Layer`, `Fix`, `FailureRecord`, `AgentResult`). Tests: `tests/unit/test_agent_types.py` valida valores de enum + frozen dataclasses.
- ⚪ **F4.2** — `feat(F4): manifest migrations for agent_runs + agent_failures` (estende `src/pipeline/state/manifest.py` com DDL + índices, `start_run`, `end_run`, `record_failure`, `record_fix`, `mark_escalated`, `count_attempts`). Tests: `tests/unit/test_agent_state.py` cobre CRUD + idempotência + counts.
