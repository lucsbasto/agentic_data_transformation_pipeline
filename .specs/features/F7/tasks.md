...
- ✅ **F7.3** — `feat(F7): secret redaction in structlog processor`. Shipped 83b7210 — `redact_secrets()` + `redact_secrets_processor()` em `pipeline.agent._logging`; recurses dict/list/tuple; key-pattern `*(_KEY|_TOKEN|_SECRET|_PASSWORD)$` (case-insensitive) OR value-pattern `sk-[A-Za-z0-9]{20,}`. Wired into `AgentEventLogger.event()` (both JSONL + stdout sinks). 7 new tests em `tests/unit/test_agent_logging.py`.
