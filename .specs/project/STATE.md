...
- **D-016 (2026-04-23, F2):** Manifest Silver = **nova tabela `silver_runs`** com FK p/ `batches` (ON DELETE CASCADE), não colunas em `batches`. Motivo: extensível p/ `gold_runs` em F3, mantém `batches` append-only, permite múltiplos Silver rebuilds por Bronze batch (futuro).
